    ...
    getLayover = (item1, item2) => {
        ...
        if (item1) {
            let data = <div className="row">
                <div className="col s12 m12 l12"><div className="col s12 m12 l12 center-align"><i className="large material-icons" style={{ fontSize: '20px', color: "red" }}>access_time</i>{`  ${this.getAirport(item1.origin.airport).location}  `}{DurationCalculate(item1.departs_at, item2.arrives_at)}  Layover</div></div>
            </div>;
            return data;
        }
        else {
            ...
            <div className="col s12 m12 l12">
                <div className="card-panel" style={{ marginBottom: '1px' }}>
                    ...
                    <div className="row" style={{ marginBottom: 5 }}>
                        <span style={{ fontSize: '20px' }} >{this.props.flightDetail[0].outbound.flights[0].origin.airport}  -  {this.props.flightDetail[0].outbound.flights[this.props.flightDetail[0].outbound.flights.length - 1].destination.airport} {`   ${new Date(this.props.flightDetail[0].outbound.flights[0].departs_at).toDateString()}`}</span>{refundable}
                    </div>
                    <div className="col s12 m12 l12">
