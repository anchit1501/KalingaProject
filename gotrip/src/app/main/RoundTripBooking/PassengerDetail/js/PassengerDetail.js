...
import FullScreenSpinner from '../../spinner/js/FullScreenSpinner';
...
class PassengerDetail extends Component {
  ...
  PostBooking = (result, flag) => {
    console.log(result);
    if (flag) {

      result = JSON.parse(sessionStorage.getItem('outbound'));

    }
    else {
      result = JSON.parse(sessionStorage.getItem('inbound'));
    }
    ...
    let data = {
      ...
      total_price: result.fare.total_price + this.props.addons,
      total_fare_flight: result.fare.total_price,
    ...
    fetch(baseurl + 'booking', {
      ...
      .then(response => {
        ...
        if (flag) {
          this.setState({ post_outbound: true });
        }
        else {
          this.setState({ post_inbound: true });
        }
      });
  }
  ...
  HandleSubmit = (event) => {
    event.preventDefault();
    console.log(this.props.roundout);
    let total = 0;


    let resultin = JSON.parse(sessionStorage.getItem('outbound'));

    let resultout = JSON.parse(sessionStorage.getItem('inbound'));

    total = (parseFloat(resultin.fare.total_price) + parseFloat(resultout.fare.total_price)) * this.props.total_passenger;
    // console.log(this.props.addons);
    // return false;
    this.setState({ booking_button: true });
    this.PostBooking(resultin, false, total);
  ...
  render() {
    ...
    let button = '';
    if (item != null) {
      button = <div className="col s12 m7 l7" style={{ marginTop: "10px" }}><button className="btn btn-default right" type="submit" disabled={this.state.booking_button} >PayNow</button></div>;
    }
    else {
    ...
    if (this.state.post_inbound && this.state.post_outbound) {
